...
No Semantic Model exists in SADP — columns are fetched live from Snowflake.
Reuses the Snowflake connection from the Depot step if available.
"""
...
def _manual_key():
    return f"sadp_qc_manual_{st.session_state.get('sadp_qc_sf_last_table', '')}"

def _accepted_manual_key():
    return f"sadp_qc_acc_manual_{st.session_state.get('sadp_qc_sf_last_table', '')}"
...
conn = st.session_state.sadp_qc_sf_conn

d1, d2, d3 = st.columns(3)
with d1:
    db_opts = ["— select —"] + st.session_state.sadp_qc_sf_databases
    db_idx  = db_opts.index(st.session_state.sadp_qc_sf_last_db) if st.session_state.sadp_qc_sf_last_db in db_opts else 0
    sel_db  = st.selectbox("Database", db_opts, index=db_idx)
    if sel_db != "— select —" and sel_db != st.session_state.sadp_qc_sf_last_db:
        st.session_state.sadp_qc_sf_last_db     = sel_db
        st.session_state.sadp_qc_sf_schemas     = fetch_schemas(conn, sel_db)
        st.session_state.sadp_qc_sf_last_schema = ""
        st.session_state.sadp_qc_sf_tables      = []
        st.session_state.sadp_qc_sf_last_table  = ""
        reset_table_state()
        st.rerun()

with d2:
    sc_opts = ["— select —"] + st.session_state.sadp_qc_sf_schemas
    sc_idx  = sc_opts.index(st.session_state.sadp_qc_sf_last_schema) if st.session_state.sadp_qc_sf_last_schema in sc_opts else 0
    sel_sc  = st.selectbox("Schema", sc_opts, index=sc_idx, disabled=not st.session_state.sadp_qc_sf_schemas)
    if sel_sc != "— select —" and sel_sc != st.session_state.sadp_qc_sf_last_schema:
        st.session_state.sadp_qc_sf_last_schema = sel_sc
        st.session_state.sadp_qc_sf_tables      = fetch_tables(conn, st.session_state.sadp_qc_sf_last_db, sel_sc)
        st.session_state.sadp_qc_sf_last_table  = ""
        reset_table_state()
        st.rerun()

with d3:
    tb_opts = ["— select —"] + st.session_state.sadp_qc_sf_tables
    tb_idx  = tb_opts.index(st.session_state.sadp_qc_sf_last_table) if st.session_state.sadp_qc_sf_last_table in tb_opts else 0
    sel_tb  = st.selectbox("Table", tb_opts, index=tb_idx, disabled=not st.session_state.sadp_qc_sf_tables)
    if sel_tb != "— select —" and sel_tb != st.session_state.sadp_qc_sf_last_table:
        st.session_state.sadp_qc_sf_last_table = sel_tb
        reset_table_state()
        with st.spinner(f"Pulling schema & profiling for {sel_tb}..."):
            try:
                _ctx = fetch_full_context(conn, st.session_state.sadp_qc_sf_last_db, st.session_state.sadp_qc_sf_last_schema, sel_tb)
                _ctx["schema_overview"] = fetch_schema_overview(conn, st.session_state.sadp_qc_sf_last_db, st.session_state.sadp_qc_sf_last_schema)
...
if not st.session_state.sadp_qc_ctx:
    st.stop()
...
approved_file = st.file_uploader("Upload reviewed Excel file", type=["xlsx"], key="sadp_approved_upload")
...
with st.form("sadp_meta_form"):
    m1, m2 = st.columns(2)
    with m1:
        wf_name = st.text_input("Workflow Name *", value=st.session_state.sadp_qc_wf_name or f"soda-{ctx['table'].lower()}-qc", placeholder="e.g. soda-sales-qc")
        wf_desc = st.text_area("Description", value=st.session_state.sadp_qc_wf_desc or f"Quality checks for {ctx['table']}", height=80)
    with m2:
    ...
    with co1:
        wf_engine  = st.text_input("Engine (optional)", value=st.session_state.sadp_qc_wf_engine, placeholder="minerva")
    with co2:
        wf_cluster = st.text_input("Cluster (optional)", value=st.session_state.sadp_qc_wf_cluster)
    st.markdown("**Tags**")
    t1, t2, t3 = st.columns(3)
    with t1:
        tag_domain  = st.text_input("DPDomain", value=st.session_state.sadp_qc_wf_tag_domain, placeholder="Sales")
    ...
    if gen_btn:
        if not wf_name.strip():
            ...
            st.session_state.sadp_qc_wf_name      = wf_name.strip()
            st.session_state.sadp_qc_wf_desc      = wf_desc.strip()
            st.session_state.sadp_qc_wf_depot     = wf_depot.strip()
            st.session_state.sadp_qc_wf_workspace = wf_workspace.strip()
            st.session_state.sadp_qc_wf_engine    = wf_engine.strip()
            st.session_state.sadp_qc_wf_cluster   = wf_cluster.strip()
            ...
            db  = st.session_state.sadp_qc_sf_last_db
            sch = st.session_state.sadp_qc_sf_last_schema
