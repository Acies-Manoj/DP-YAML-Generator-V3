...
# Get all QC files
_qc_all = st.session_state.get("cadp_qc_all_yaml", {})
...
if (step == 1
    ...
    st.session_state.dp_bundle_qc_resources = [
        {
            "file": f"build/quality-checks/{data['file_name']}",
            "workspace": "public"
        }
        for data in _qc_all.values()
    ]
...
if step == 1:
    ...
    if not st.session_state.dp_preview_mode:
        ...
        if _qc_all and not _cadp_qc_skipped and st.session_state.dp_bundle_qc_resources:
            file_list = [data["file_name"] for data in _qc_all.values()]
            st.info(
                f"✅ {len(file_list)} QC files added:\n\n" +
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with zipfile.ZipFile(zip_buf, "w") as zf:
        ...
        zf.writestr(f"scan-{spec_name}.yml", scanner_yaml)
         # ✅ ADD THIS BLOCK (QC FILES)
        qc_all = st.session_state.get("cadp_qc_all_yaml", {})

        for table, data in qc_all.items():
            zf.writestr(
                f"quality-checks/{data['file_name']}",
                data["content"]
            )
    zip_buf.seek(0)
